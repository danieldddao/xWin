...
namespace xWin.Library
{
    ...
    public struct ControllerCalibration
    {
        ...
        public uint deadzone, t_deadzone;
    }
    ...
    public class WI//Windows Interaction
    {
        ...
        public static void MoveCursor_Dynamic(short x, short y, int dpi = 10)
        {
            const float MAX_INPUT = 32767;
            float xDiff = 0;
            float yDiff = 0;
            ...
            Cursor.Position = new Point(Cursor.Position.X + (short)Math.Floor(xDiff), Cursor.Position.Y+(short)Math.Floor(yDiff));
        ...
        public static void release_all()
        {
            ...
            {
                if(k != Keys.Control && k != Keys.ShiftKey && k != Keys.Alt)
                {
                    Release(k);
                }
            }
        ...
        public static void InteractionLoop()
        {
            ...
            try {
                while (true)
                {
                    ...
                    {
                        if (Math.Sqrt(datas.Gamepad.LeftThumbX * datas.Gamepad.LeftThumbX + datas.Gamepad.LeftThumbY * datas.Gamepad.LeftThumbY) < cc.t_deadzone)
                        {
                            datas.Gamepad.LeftThumbX = 0;
                            datas.Gamepad.LeftThumbY = 0;
                        }
                        datas.Gamepad.RightThumbX = shortbound(datas.Gamepad.RightThumbX - cc.rx);
                        datas.Gamepad.RightThumbY = shortbound(datas.Gamepad.RightThumbY - cc.ry);
                        if (Math.Sqrt(datas.Gamepad.RightThumbX * datas.Gamepad.RightThumbX + datas.Gamepad.RightThumbY * datas.Gamepad.RightThumbY) < cc.t_deadzone)
                        {
                            datas.Gamepad.RightThumbX = 0;
                            datas.Gamepad.RightThumbY = 0;
                        }
                        ...
                        case Mode.Normal:
                            {
                                ...
                                foreach (var l in kms.pressed.Reverse())
                                {
                                    Console.Write(l.ToString() + ",");
                                    switch (l)
                                    {
...
#warning OPEN MENU CODE NOT DEFINED
                                ...
                                if (M_Left)
                                    kms.mouse_movement.x -= 32767;
                                if (Program.dynamicmode)
                                    MoveCursor_Dynamic(shortbound(kms.mouse_movement.x), shortbound(kms.mouse_movement.y), (int)dpi);
                                else
